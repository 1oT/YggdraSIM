...
def test_reader_bar_activate_auto_opens_yellow_pill() -> None:
    """Clicking a yellow pill (card present, no session) must auto-open
    the scan so the pill feels like an active session switcher."""
    js = _read("app.js")
    start = js.index("function readerBarActivate(")
    remainder = js[start:]
    next_fn = remainder.index("\n  function ")
    body = remainder[:next_fn]
    assert 'readerBarDeriveStatus(readerName)' in body
    assert '"yellow"' in body, "only yellow pills auto-open"
