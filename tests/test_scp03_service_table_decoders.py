...
# Universal GUI surface is being stabilised. The frontend bundle
# assertions below degrade to ``skipped`` on checkouts where the
# directory is absent so the suite stays green.
_GUI_SERVER_TREE_AVAILABLE = _STATIC.parent.is_dir()
_GUI_SERVER_SKIP_REASON = (
    "yggdrasim_common/gui_server/ is absent on this checkout; "
    "skipping frontend bundle assertions."
)
...
def test_decode_ust_returns_active_and_inactive() -> None:
    ...
    # the body with zeros to reach 14 bytes (matches the screenshot
    # the operator pasted).
    sample_hex = "02" + "00" * 13
...
def test_app_js_has_service_table_detector_and_renderer() -> None:
    ...
    detector_pos = js.index("isServiceTablePayload(value)")
    array_pos = js.index("if (Array.isArray(value)) {")
