...
def _load_runtime_components():
    try:
        ...
        from SCP11.relay.config import SGPConfig
        from SCP11.relay.console import SCP11Console
        from SCP11.relay.factory import build_apdu_channel, build_profile_provider
        from SCP11.relay.models import TRANSPORT_MODE_PCSC
        from SCP11.relay.orchestrator import SGP22Orchestrator
    return (
        SGPConfig,
        SCP11Console,
        build_apdu_channel,
...
class SGP22Client:
    """Relay SCP11 entrypoint preserving the original shell contract."""
    ...
    def _run_startup_preflight(self):
        ...
        if errors:
            ...
            self._raise_startup_error("SCP11 startup preflight failed.", combined)
    ...
    def _build_runtime(self):
        ...
        try:
            ...
            raise SCP11StartupError(f"SCP11 transport initialization failed: {error}") from error
        self._apply_global_debug()

        try:
            self.profile_provider = self._build_profile_provider(self.cfg)
        except Exception as error:
            raise SCP11StartupError(f"SCP11 backend initialization failed: {error}") from error
...
def entry() -> None:
    ...
    add_debug_argument(
        parser,
        help_text="Enable verbose debug output for this SCP11 relay session.",
    )
    parser.add_argument(
        "--flow",
        action="store_true",
        help="Run one-shot SCP11 relay flow instead of interactive shell",
    )
    parser.add_argument(
        "--cmd",
        type=str,
