...
Bug-fixes, spec-correctness work, and API additions should land here
first. ``SCP11/live/orchestrator.py`` and ``SCP11/test/orchestrator.py``
mirror this implementation with variant-specific overlays (e.g.
``stk_polling`` mixin for live, extra request shaping for the test tree)
and are treated as *legacy mirrors*. Any change made here should be
evaluated against both mirrors; the long-term goal is to turn the mirrors
into thin shim packages that import from this module and only override
the variant delta.
"""
...
class SGP22Orchestrator:
    ...
    def _should_include_initial_eim_challenge(self, eim_fqdn: str, variant: int) -> bool:
        ...
        for suffix in self._vendor_quirk_fqdn_suffixes():
            if normalized.endswith(suffix):
                return True
        return False

    def _vendor_quirk_fqdn_suffixes(self) -> tuple[str, ...]:
