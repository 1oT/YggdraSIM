...
"""SCP11 ES9+ client: HTTP/JSON transport for the LPA → SM-DP+ interface (SGP.22 §5.5, SGP.32 §4.2)."""
# -----------------------------------------------------------------------------
...
class Es9LikeClient:
    ...
    def __init__(
        ...
        ensure_seeded_runtime_file("SCP11", "es9_ca_lookup.json")
        self._module_dir = runtime_path("SCP11")
    ...
    def _dispatch_eim_request(self, request_obj: EimPollRequest) -> dict:
        ...
        if request_obj.raw_body is not None and len(request_obj.raw_body) > 0:
            response = self._post_eim_binary(
                base_url,
                request_obj.raw_body,
                request_obj.trusted_tls_public_key_data,
            )
        else:
            response = self._poll_eim_json(base_url, request_obj)
        return response

    def _decode_eim_poll_response(self, response: dict) -> EimPollResponse:
        response_wrappers = ("body", "data", "getEimPackageResponse", "getEimPackageOk")
        ...
        if len(package_list) == 0:
            for wrapper in response_wrappers:
        ...
        ack_sequence_numbers = self._list_of_ints_field(response, "ackSequenceNumbers")
        if len(ack_sequence_numbers) == 0:
            for wrapper in response_wrappers:
                wrapped = response.get(wrapper)
                if isinstance(wrapped, dict) is False:
                    continue
                ack_sequence_numbers = self._list_of_ints_field(wrapped, "ackSequenceNumbers")
                if len(ack_sequence_numbers) > 0:
                    break
        return EimPollResponse(
            ...
            ack_sequence_numbers=ack_sequence_numbers,
    ...
    def _parse_eim_ack_sequence_numbers(self, raw: bytes) -> list[int]:
        if len(raw) == 0:
            return []
        tlv, _ = self._read_tlv_at(raw, 0)
        if tlv is None:
            return []
        ...
        while offset < len(value):
            item_tlv, next_offset = self._read_tlv_at(value, offset)
            if item_tlv is None:
                break
            item_tag, item_value, _ = item_tlv
            offset = next_offset
            if item_tag != b"\x80" or len(item_value) == 0:
                continue
            sequence_numbers.append(int.from_bytes(item_value, "big", signed=False))
        ...
        while idx < len(raw):
            ...
            if tag_bytes == b"\xBF\x50":
                ...
                if len(value) >= 3 and value.startswith(b"\xBF\x53"):
                    ...
                    out["ackSequenceNumbers"] = self._parse_eim_ack_sequence_numbers(value)
                    continue
            ...
            if tag_bytes == b"\xBF\x53":
                out["pollingComplete"] = True
                out["packageFormat"] = "eimAcknowledgements"
                out["ackSequenceNumbers"] = self._parse_eim_ack_sequence_numbers(raw_tlv)
                continue
            ...
            if (
                ...
                if len(out.get("ackSequenceNumbers", [])) == 0 and inner.get("ackSequenceNumbers"):
                    out["ackSequenceNumbers"] = list(inner["ackSequenceNumbers"])
    ...
    def _list_of_ints_field(self, payload: dict, *keys: str) -> list[int]:
        for key in keys:
            value = payload.get(key)
