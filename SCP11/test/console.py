...
class SessionPolicy:
    ...
    Mirrors :class:`SCP11.live.console.SessionPolicy`. A command
    dispatched by :class:`SCP11Console` is classified into one of three
    buckets that govern how much of the card session is torn down before
    the handler runs. The policy is applied by
...
class SCP11Console:
    ...
    TAG_AID = 0x4F
    TAG_ICCID = 0x5A
    MODULE_STATE_NAME = "scp11_test_config"
    ...
    def __init__(self, client: Any):
        ...
        self.current_eid = ""
        self._inventory = EidInventoryNamespace("scp11_test")
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                try:
                    raw_line = input(
                        f"\n{self._style.header}[eSIM Test] > {self._style.end}"
                    ).strip()
    ...
    def _cmd_eim_poll(self, argument: str) -> bool:
        try:
            dispatch_poll_command("scp11.test", "POLL", self, argument)
        except KeyboardInterrupt:
    ...
    def _fetch_server_leaf_certificate(self, hostname: str, port: int) -> Optional[bytes]:
        context = create_introspection_context(caller="SCP11.test.console/fetch_leaf")
        try:
            with socket.create_connection((hostname, port), timeout=8) as tcp_socket:
    ...
    def _decode_leaf_certificate_dict(self, hostname: str, port: int) -> Optional[Dict[str, Any]]:
        context = create_introspection_context(caller="SCP11.test.console/decode_leaf")
        try:
            with socket.create_connection((hostname, port), timeout=8) as tcp_socket:
                with context.wrap_socket(tcp_socket, server_hostname=hostname) as tls_socket:
    ...
    def _queue_modem_refresh(self, action_label: str, mode: str = "") -> None:
        try:
            payload = trigger_card_relay_modem_refresh(
                mode=mode,
                source=f"scp11-test:{action_label}",
            )
        except Exception as error:
