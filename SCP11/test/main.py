...
def _load_runtime_components():
    try:
        ...
        from SCP11.test.console import SCP11Console
        from SCP11.test.factory import build_apdu_channel, build_profile_provider
        from SCP11.test.models import TRANSPORT_MODE_PCSC
...
class SGP22Client:
    """Test-certificate SCP11 relay entrypoint."""
    ...
    def _run_startup_preflight(self):
        ...
        if errors:
            ...
            self._raise_startup_error("SCP11 test startup preflight failed.", combined)
    ...
    def _build_runtime(self):
        ...
        try:
            ...
            raise SCP11StartupError(f"SCP11 test backend initialization failed: {error}") from error
...
def entry() -> None:
    ...
    parser.add_argument(
        "--flow",
        action="store_true",
        help="Run one-shot SCP11 test relay flow instead of interactive shell",
    )
    parser.add_argument(
        "--cmd",
        type=str,
