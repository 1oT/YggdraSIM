...
class LocalIsdrSession:
    ...
    def collect_quick_overview(self) -> dict[str, Any]:
        ...
        # entries on the header card, so it is kept. ``GetCerts`` and
    ...
    def run_load_profile_chain(self, profile_path: str = "") -> bytes:
        ...
        Transaction ID and card expectations: the transaction ID from the BPP file is used
    ...
    def _derive_metadata_document_from_profile(self, profile_bytes: bytes) -> dict[str, Any]:
        ...
        # still the first TLV and already carries the required identity fields.
    ...
    def _progress_coast_remaining_segments(
        ...
        loop, fast-forward the bar across the segments that will not be
    ...
    def _extract_notification_metadata_entries(self, raw_response: bytes) -> list[dict[str, Optional[int]]]:
        ...
        if isinstance(decoded, tuple) is False or len(decoded) != 2:
            return entries
        choice_name, choice_value = decoded
        if choice_name != "notificationMetadataList" or isinstance(choice_value, list) is False:
            return entries
