...
This module is a ``legacy mirror`` retained for the relay-first,
live-certificate default flow and adds the ``LiveStkPollingMixin``
overlay on top of the shared behaviour. Do not fix spec issues here in
isolation -- mirror any change against the canonical tree and (where it
applies) ``SCP11/test/orchestrator.py``. A future cleanup consolidates
this mirror into a thin shim package.
"""
...
class SGP22Orchestrator:
                    ...
                    if total_entries > 1:
                        ...
                        for label_text, error_text in entry_failures.values():
                            if len(error_text) > 0:
                                ...
                                print(f"    - {label_text}")
                    # The sweep does not raise when every configured entry
                    # fails; instead the ``_last_eim_poll_reached_server``
                    # flag carries that signal to the console without
                    # short-circuiting notification sync or other post-flow
                    # steps.
        finally:
    ...
    def _progress_coast_remaining_segments(
        ...
        loop, fast-forward the bar over the segments that will not be
