...
class PcscApduChannel:
    ...
    def exchange(self, apdu: bytes, log_name: str) -> Tuple[bytes, int, int]:
        ...
        # ``getattr`` with a default lets unit tests build channels via
        # ``PcscApduChannel.__new__`` (bypassing ``__init__`` to skip the
        # PC/SC reader probe on a CI runner) and still hit ``exchange``
        # without having to re-set every private flag by hand.
        # Subclasses that forget the attribute also degrade cleanly to
        # the verbose path rather than raising ``AttributeError`` deep
        # in the middle of an APDU exchange.
        quiet = bool(getattr(self, "_quiet_apdu_logging", False))
        raw = bool(getattr(self, "_raw_apdu_logging", False))
        if quiet is False:
