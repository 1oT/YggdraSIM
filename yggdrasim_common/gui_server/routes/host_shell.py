...
* ``GET  /api/host-shell/capabilities`` — capability snapshot the SPA
  consults before rendering the sidebar entry. ``?scope=hil-modem``
  reports the constrained serial-terminal capability used by the HIL
  module.
...
def get_capabilities(scope: str = "") -> dict:
    """Return the host-shell capability snapshot."""
    if str(scope or "").strip().lower() == "hil-modem":
        return host_shell_module.describe_hil_modem_capability()
        ...
        await websocket.send_text(
            json.dumps({
                ...
                "message": (
                    ...
                    "and restart yggdrasim-gui or yggdrasim-web-server to enable it."
                ),
            })
        )
