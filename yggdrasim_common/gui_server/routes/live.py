...
def _scp03_session_rows_by_reader_name() -> dict[str, list[dict[str, Any]]]:
    """Map reader-name to live SCP03 session rows."""
    ...
    try:
        for entry in get_manager().list():
            ...
            if not reader_name or reader_name == "(default)":
                continue
            mapping.setdefault(reader_name, []).append(dict(entry))
...
def _session_atr_by_reader_name() -> dict[str, str]:
    ...
    The fix is two-pronged:
      1. Cache the ATR from the live SCP03 session so a transient
         sharing-layer probe error does not make an active card vanish.
      2. Probe with ``SCARD_LEAVE_CARD`` so the probe's ``disconnect()``
         doesn't power-cycle the card (see ``_probe_reader``). A real
         no-card probe result closes stale sessions for that reader.
    """
    mapping: dict[str, str] = {}
    for reader_name, rows in _scp03_session_rows_by_reader_name().items():
        for entry in rows:
            md = entry.get("metadata") or {}
...
def _close_scp03_sessions_for_reader_name(reader_name: str) -> int:
    """Close stale SCP03 sessions bound to ``reader_name``."""
    name = str(reader_name or "").strip()
    if len(name) == 0:
        return 0
    try:
        from yggdrasim_common.gui_server.sessions import get_manager
    ...
    manager = get_manager()
    closed = 0
    for entry in _scp03_session_rows_by_reader_name().get(name, []):
        session_id = str(entry.get("id") or "").strip()
        if session_id and manager.close(session_id):
            closed += 1
    if closed:
        _LOGGER.info("closed %d stale SCP03 session(s) for empty reader %s", closed, name)
    return closed


def _is_no_card_probe_error(error: BaseException) -> bool:
    """Return True for PC/SC errors that mean an empty reader slot."""
    name = type(error).__name__
    if name in ("NoCardException",):
        return True
    text = str(error)
    return "0x8010000C" in text or "No smart card inserted" in text
...
def _remote_row_duplicates_local_reader(remote_row: ReaderInfo | None, local_names: list[str]) -> bool:
    """Return True when a remote bridge row represents a local PC/SC reader.

    HIL exposes the same physical reader twice while the APDU relay is
    configured: once as the local PC/SC reader and once as the remote
    relay status row. The top-bar session strip must not show both as
    independent reader sessions.
    """
    if remote_row is None:
        return False
    if remote_row.kind != "remote":
        return False
    remote_name = str(remote_row.name or "").casefold()
    for local_name in local_names:
        cleaned = str(local_name or "").strip()
        if len(cleaned) == 0:
            continue
        if cleaned.casefold() in remote_name:
            return True
    return False
...
def list_readers() -> ReadersResponse:
    ...
    for reader in reader_list:
        ...
        if cached_atr and len(info.atr_hex.strip()) == 0:
            _close_scp03_sessions_for_reader_name(str(reader))
        rows.append(info)

    local_names = [str(reader) for reader in reader_list]
    remote_duplicate = _remote_row_duplicates_local_reader(remote_row, local_names)

    if remote_row is not None and not remote_duplicate:
        rows.append(remote_row)

    local_count = len(rows) - (1 if remote_row is not None and not remote_duplicate else 0)
    if remote_row is not None and not remote_duplicate:
        note = f"found {local_count} local reader(s) + 1 remote bridge."
    elif remote_duplicate:
        note = f"found {local_count} local reader(s); remote bridge maps to an existing reader."
    else:
...
def _probe_reader(reader: Any, *, cached_atr: str = "") -> ReaderInfo:
    ...
    * **Cached ATR is a fallback, not presence proof** — if
      ``cached_atr`` is non-empty (populated from a live scp03
      session's metadata in the caller), we still validate the reader.
      Otherwise a removed card can stay green forever in the top bar.
      The cache is only reused for non-presence probe errors.
    ...
    * **``SCARD_LEAVE_CARD`` on disconnect** — pyscard's
    ...
    try:
        ...
        if cached_atr and not _is_no_card_probe_error(error):
            return ReaderInfo(
                name=name,
                atr_hex=cached_atr,
                status=f"card present (cached; probe error: {error})",
            )
