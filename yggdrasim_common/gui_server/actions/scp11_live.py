...
# Copyright (c) 2026 1oT OÜ. Authored by Hampus Hellsberg.
"""SCP11 eSIM management Command Center actions.
...
deliberately short-lived: each call opens a fresh PC/SC channel, runs a
    single console command (either directly on the orchestrator or via a
...
def _provider_module(submodule: str) -> Any:
    """Import a module from the single eSIM management relay provider."""
    return importlib.import_module(f"SCP11.live.{submodule}")
...
def _build_euicc_info2_lines(raw: bytes) -> list[dict[str, Any]]:
    ...
    for entry in detail:
        if isinstance(entry, dict):
            ...
            indent = int(entry[0]) if len(entry) > 2 else 0
            label_text = str(entry[1] or "").strip()
            value_text = str(entry[2] or "").strip() if len(entry) > 2 else ""
        else:
            label_text = ""
...
def _build_certs_lines(raw: bytes) -> list[dict[str, Any]]:
    ...
    if euicc_present:
        lines.append({
            "label": "eUICC Certificate DER Hex",
            "value": euicc.hex().upper(),
            "indent": 1,
        })
    return lines
...
class _FakeClient:
    ...
    The GUI needs the same provider and transport factory hooks as the CLI
    client so the console's command-session policy can rebuild card
    transport between flows.
    """
    ...
    def __init__(
        ...
        build_apdu_channel: Callable[[Any], Any],
        build_profile_provider: Callable[[Any], Any],
    ) -> None:
...
def _build_console(reader_index: int) -> tuple[Any, Any]:
    ...
    console is never driven through ``run()`` / ``run_commands()``. Registered
    stream actions still use ``_execute_command`` so the shell and GUI share
    the same command-session policy.
    """
    SGPConfig = getattr(_provider_module("config"), "SGPConfig")
    SCP11Console = getattr(_provider_module("console"), "SCP11Console")
    build_profile_provider = getattr(_provider_module("factory"), "build_profile_provider")
    SGP22Orchestrator = getattr(_provider_module("orchestrator"), "SGP22Orchestrator")
    ...
    fake = _FakeClient(
        ...
        profile_provider=profile_provider,
        build_apdu_channel=lambda active_cfg: _build_apdu_channel(active_cfg.READER_INDEX),
        build_profile_provider=build_profile_provider,
    )
    ...
    *,
    ...
    def _worker() -> None:
        ...
        try:
            ...
            if len(command) > 0:
                try:
                    use_command_policy = command in getattr(console, "_commands", {})
                except Exception:
                    use_command_policy = False
            if connect_first and use_command_policy is False:
                try:
            ...
            try:
                with contextlib.redirect_stdout(stream_stdout), \
                    ...
                    if use_command_policy:
                        console._execute_command(command, argument)
                    else:
                        handler = getattr(console, handler_name)
                        handler(argument)
                stream_stdout.flush()
...
def _card_snapshot_payload(snapshot: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "configured_decoded": dict(getattr(snapshot, "configured_decoded", {}) or {}),
    }
...
def _dispatch_scan(
    ...
    *,
    ...
    def _work(console: Any) -> dict[str, Any]:
        return _card_snapshot_payload(console._collect_start_snapshot())
...
def _dispatch_get_metadata(
    ...
    *,
    ...
    """Return profile metadata for all profiles or one selected profile."""
    identifier = str(target or "").strip()
    if identifier.lower() in ("", "all", "*"):
        result = _dispatch_read_metadata(ctx, reader=reader)
        return {
            **result,
            "mode": "all",
            "target": "ALL",
            "found": bool(result.get("rows")),
            "metadata": {},
        }
...
def _dispatch_get_all_data(
    ...
    *,
    ...
    def _capture_step(callable_: Callable[..., Any], *args: Any) -> tuple[Any, str, str]:
        sink = io.StringIO()
        try:
            with contextlib.redirect_stdout(sink), contextlib.redirect_stderr(sink):
            ...
            return None, _strip_ansi(sink.getvalue()), f"{type(error).__name__}: {error}"

    def _work(console: Any) -> dict[str, Any]:
        _, action_report, action_error = _capture_step(console._cmd_get_all_data, "")
        collected_snapshot: dict[str, Any] = {}
        snapshot_obj, _, snapshot_error = _capture_step(console._collect_start_snapshot)
        if snapshot_obj is not None:
            collected_snapshot = _card_snapshot_payload(snapshot_obj)
        return {
            "report": action_report,
            "snapshot": collected_snapshot,
            "errors": [err for err in (action_error, snapshot_error) if err],
        }

    try:
        result, trace = _run_console_callable(reader_index, _work, connect_first=True)
        report = str(result.get("report", "") if isinstance(result, dict) else "")
        snapshot = dict(result.get("snapshot", {}) if isinstance(result, dict) else {})
        if isinstance(result, dict):
            note_parts.extend(str(err) for err in result.get("errors", []) if err)
        if trace.strip():
...
def _dispatch_get_es9(ctx: ActionContext) -> dict[str, Any]:
    """Return the ES9 cfg view for the eSIM management relay."""
    ...
    return {
        ...
        "validation_lines": [],
        "input_length": 0,
        "note": "ok" if cfg is not None else "SCP11.live.config unavailable",
    }
...
READ_METADATA_SPEC = ActionSpec(
    ...
    subsystem="eSIM Management",
    title="Get all profile metadata",
    description=(
...
GET_METADATA_SPEC = ActionSpec(
    ...
    description=(
        "Return metadata for every profile by default, or for a single "
        "profile when an ICCID, ISD-P AID, or alias is provided."
    ),
    inputs=(
        _reader_input_field(),
        ActionField(
            name="target",
            label="Target (optional ICCID / AID / alias)",
            kind="string",
            required=False,
            default="",
            placeholder="blank or ALL = every profile",
            help=(
                "Leave blank or enter ALL for every profile; enter an "
                "ISD-P AID, ICCID, or alias for one profile."
            ),
        ),
    ),
    output_kind="json",
...
EIM_DOWNLOAD_SPEC = ActionSpec(
    ...
    description=(
        "Run the eIM poll+relay flow. May queue REFRESH and rewrite notifications."
    ),
    inputs=(
        _reader_input_field(),
    ),
