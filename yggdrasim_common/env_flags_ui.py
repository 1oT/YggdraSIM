...
module does not import ``main.main`` at all. ``main/main.py`` is
executed as ``__main__`` and ``main/`` is not a proper Python
package at runtime, so this module lives under ``yggdrasim_common``
to guarantee PyInstaller's submodule collection picks it up.
