...
def _emit_gfm_filesystem_rows(
    ...
    SELECT cursor semantics (matches pySim
    :py:meth:`FsProfileElement.pe2files`, lines 771-807):
...
def _dispatch_update_file_field(
    ...
    *,
    ...
    choice_list = pe.decoded.get(path_text)
    ...
    refreshed_section = handle["decoded_document"]["sections"].get(section) or {}
    refreshed_row = _summarise_file_choices(refreshed_section.get(path_text))
    return {
        "session_id": sid,
        "section_key": section,
        "field_path": path_text,
...
def _build_default_pe(pe_type: str) -> Any:
...
def _dispatch_add_pe(
    ...
    *,
    ...
    new_pe = _build_default_pe(pe_type_text)
...
def _dispatch_update_profile_header_field(
    ...
    * ``iccid_digits`` — 19 or 20 decimal digits (TS 102 221 §13.2 BCD).
    * ``iccid_hex`` — 20-nybble pre-swapped hex (advanced override).
...
UPDATE_PROFILE_HEADER_FIELD_SPEC = ActionSpec(
    ...
    description=(
        ...
        "profile_type, iccid_digits, iccid_hex, pol_hex, "
        "connectivity_parameters_hex, iot_pix_hex. Empty value clears "
    ...
    inputs=(
        ...
        ActionField(
            ...
            kind="string",
            required=True,
            help="One of profile_type / iccid_digits / iccid_hex / pol_hex / connectivity_parameters_hex / iot_pix_hex.",
        ),
        ActionField(
            name="value",
            label="New value",
