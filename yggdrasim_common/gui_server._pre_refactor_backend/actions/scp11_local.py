...
# ----------------------------------------------------------------------
# Additional dispatchers (status, load_profile, refresh_modem,
...
def _dispatch_refresh_modem(
    ...
    *,
    ...
    return {
        "reader_name": _reader_label(str(reader or "")),
...
REFRESH_MODEM_SPEC = ActionSpec(
    id="scp11_local.refresh_modem",
    subsystem="SCP11 Local",
    title="Refresh modem",
    description=(
        "Queue a proactive REFRESH toward the modem so it re-reads "
        "the profile list from the card after enable/disable/delete."
    ),
    inputs=(
        _reader_input_field(),
        ActionField(
    ...
    requires_card=False,
    streams=False,
    tags=("scp11", "local", "modem", "refresh"),
)
