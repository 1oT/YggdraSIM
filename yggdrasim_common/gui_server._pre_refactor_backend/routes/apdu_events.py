...
async def apdu_event_stream(websocket: WebSocket) -> None:
    ...
    queue: asyncio.Queue[ApduExchange] = asyncio.Queue(maxsize=512)
    ...
    try:
        ...
        # Periodically push a heartbeat so the SPA can detect a stalled
        # transport even when no APDUs are flowing. The frame carries
        # ``event=ping`` and is ignored by the bus relay.
